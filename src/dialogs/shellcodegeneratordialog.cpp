...
struct StyleEntry { const char* label; int id; };
...
ShellcodeGeneratorDialog::ShellcodeGeneratorDialog(QWidget* parent)
    ...
    for (int i = 0; i < kArchCount; ++i) m_archCombo->addItem(kArchEntries[i].label);
    ...
    for (int i = 0; i < kStyleCount; ++i) m_shellcodeStyle->addItem(kStyles[i].label, kStyles[i].id);
    ...
    auto triggerReassemble = [this](int) { if (!m_shellcodeOutput->toPlainText().isEmpty()) onAssemble(); };
...
void ShellcodeGeneratorDialog::onAssemble()
{
    ...
    switch (styleId) {
    case 0:  output = generateC(raw, bits);   break;
    case 1:  output = generateCpp(raw, bits); break;
    case 2:  output = generateRaw(raw);       break;
    default: output = generateC(raw, bits);
    }
...
QList<ShellcodeGeneratorDialog::DisasmEntry>
ShellcodeGeneratorDialog::disassemble(const QByteArray& raw, const QString& bits) const
{
    ...
    {
        QFile f(tmpBin);
        if (!f.open(QIODevice::WriteOnly)) return result;
        f.write(raw);
    }
